...
ruta_procesar = os.path.join(os.path.dirname(__file__), 'Puntos de rutas')
if os.path.exists(ruta_procesar):
    ...
    raise FileNotFoundError(f"No se encontró la carpeta 'Puntos de rutas'")
...
def calcular_consumo_y_costos_viaje(origin_lat, origin_lon, dest_lat, dest_lon, 
    ...
    return {
        ...
        'duracion_ida_min': ruta_ida.get('duracion_segundos', 0) / 60,
        'duracion_vuelta_min': ruta_vuelta.get('duracion_segundos', 0) / 60,
        ...
        'ahorro_min': ahorro_min,
        'ahorro_max': ahorro_max,
        'ahorro_promedio': (ahorro_min + ahorro_max) / 2,
        'porcentaje_ahorro_min': porcentaje_ahorro_min,
        'porcentaje_ahorro_max': porcentaje_ahorro_max,
...
def inferir_info_geo(lat, lon):
    """
    Infiere municipio y estrato a partir de lat/lon usando los shapefiles cargados.
    """
    try:
        punto = Point(lon, lat)
        
        # 1. Buscar en ZONAS SIT (Medellín AM)
        # Asume que ZONAS SIT tiene columnas 'Municipio' y 'Estrato' (o 'ESTRATO')
        if 'geometry' in zonas_sit.columns:
            # Filtro espacial vectorializado (más rápido que iterar)
            matches = zonas_sit[zonas_sit.contains(punto)]
            if len(matches) > 0:
                row = matches.iloc[0]
                mun = row.get('Municipio', 'Medellín')
                # Intentar buscar columna de estrato normalizada
                est = row.get('Estrato', row.get('ESTRATO', '3'))
                return str(mun), str(est)

        # 2. Buscar en ZAT (Bogotá)
        if 'geometry' in zat.columns:
            matches = zat[zat.contains(punto)]
            if len(matches) > 0:
                # ZAT suele ser Bogotá
                return "Bogotá", "3" # ZAT a veces no tiene estrato directo
    except Exception as e:
        print(f"Error infiriendo info geo: {e}")
